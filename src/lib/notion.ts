...
export function readNotionBreaksDatabaseId(): string {
  const raw = process.env["NOTION_BREAKS_DATABASE_ID"]?.trim();
  if (!raw) {
    throw new Error("NOTION_BREAKS_DATABASE_ID is not set");
  }

  const databaseId = extractNotionId(raw);
  if (!databaseId) {
    throw new Error(`NOTION_BREAKS_DATABASE_ID is not a valid Notion database ID: ${raw}`);
  }
